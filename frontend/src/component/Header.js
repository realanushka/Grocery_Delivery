    ...
    <header className='fixed shadow-md w-full h-20 px-2 md:px-4 z-50 bg-white'>
      {/* desktop */}
        ...
        <div className='flex items-center h-full justify-between'>
          <Link to={""}>
            <div className='h-10'>
                <img style={{ width: 175, height: 60 }} src={logo} className='h-full'/>
            </div>
          </Link>
          <div className='flex items-center gap-4 md:gap-7'>
             <nav className=' gap-4 md:gap-6 text-base md:text-lg hidden md:flex'>
              <Link to={""}>Home</Link>
              <Link to={"menu/64f1daa18e596174e967bed9"}>Menu</Link>
              ...
              { showMenu && (
                <div className='absolute right-2 bg-white py-2  shadow drop-shadow-md flex flex-col min-w-[120px] text-center'>
                  ...
                  <nav className='text-base md:text-lg flex flex-col md:hidden'>
                    ...
                    <Link to={"menu/64f1daa18e596174e967bed9"} className='px-2 py-1'>Menu</Link>
